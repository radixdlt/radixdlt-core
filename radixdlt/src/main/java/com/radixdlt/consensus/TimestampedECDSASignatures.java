...
public final class TimestampedECDSASignatures {
	...
	private ImmutableMap<ECPublicKey, Pair<Long, ECDSASignature>> keyToTimestampAndSignature;

	@JsonCreator
	public static TimestampedECDSASignatures from(@JsonProperty("signatures") Map<String, Map<Long, ECDSASignature>> signatures) {
		ImmutableMap<ECPublicKey, Pair<Long, ECDSASignature>> sigs = signatures == null
			? ImmutableMap.of()
			: signatures.entrySet().stream()
				.collect(ImmutableMap.toImmutableMap(e -> decodePublicKey(e.getKey()), e -> pairFromMap(e.getValue())));
	 ...
	 */
	public TimestampedECDSASignatures(ImmutableMap<ECPublicKey, Pair<Long, ECDSASignature>> keyToTimestampAndSignature) {
	 ...
	 */
	public Map<ECPublicKey, Pair<Long, ECDSASignature>> getSignatures() {
	...
	private static ECPublicKey decodePublicKey(String str) {
		try {
